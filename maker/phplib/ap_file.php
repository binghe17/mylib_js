...
// //----读写数据
// FileUtil::writeFile('c/a/a.php','中文写入时UTF-8，只有英文时ASCII');		//写入文件内容
// echo FileUtil::readFile('c/a/a.php');								//读取文件内容
...
class FileUtil {
	 ...
	 * @return int 写入字符串的总长度
	 */
	function writeFile($aimUrl, $fileData) {
		$aimDir = dirname($aimUrl);
		FileUtil::createDir($aimDir);
...
function DirZip($t_dir,$n_dir,$zipfile){
	...
	while(false !== ($filename = readdir($dirh))) { 
		if($filename != '.' && $filename != '..') {
			...
			} else $zipfile -> add_file($t_dir.'/'.$filename,$n_dir.'/'.$filename);
		}
	} 
	closedir($dirh);
}
